...
for n in range(7):
    def fitness_function(solution):
        ...
        model1 = Sequential()
        model1.add(LSTM(int(solution[0]), input_shape=(x_train.shape[1], x_train.shape[2])))
    ...
    dataset = pd.read_excel(io="Data_after_VMD.xlsx", sheet_name=n)
    ...
    lb = [12, 0.001]
    ub = [72, 0.01]
    Vmin = -5 * np.ones([dim, 1])
    Vmax = 5 * np.ones([dim, 1])
    fobj = fitness_function

    # Prediction model parameters optimization
    GbestScore, GbestPositon, Curve = PSO(pop, dim, lb, ub, MaxIter, fobj, Vmin, Vmax)
    ...
    model = Sequential()
    model.add(LSTM(int(GbestPositon[0]), input_shape=(x_train.shape[1], x_train.shape[2])))
