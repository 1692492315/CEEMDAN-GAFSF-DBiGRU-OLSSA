...
''' Population initialization using the oppositional learning strategy'''
def initial(pop, dim, ub, lb, fun):
    YX = np.zeros([pop, dim])
    OX = np.zeros([pop, dim])
    for i in range(pop):
        for j in range(dim):
            YX[i, j] = random.random() * (ub[j] - lb[j]) + lb[j]
            OX[i, j] = ub[j] + lb[j] - YX[i, j]
    ZX = pd.concat([pd.DataFrame(YX), pd.DataFrame(OX)], axis=0).values
    fitness = CaculateFitness(ZX, fun)
    fitness, sortIndex = SortFitness(fitness)
    ZX = SortPosition(ZX, sortIndex)
    X = ZX[0:pop, :]
    return X
...
'''Oppositional learning sparrow search algorithm'''
def OLSSA(pop, dim, lb, ub, Max_iter, fun):
    ST = 0.6
    PD = 0.7
    SD = 0.2
    PDNumber = int(pop * PD)
    SDNumber = int(pop * SD)
    X = initial(pop, dim, ub, lb, fun)
