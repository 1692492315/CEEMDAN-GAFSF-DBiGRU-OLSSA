...
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=2, input_shape=(x_train.shape[1], x_train.shape[2])))
model.add(MaxPooling1D(pool_size=2))
model.add(Flatten())
model.add(Dense(100))
model.add(Dense(1))
model.compile(optimizer=tf.keras.optimizers.Adam(0.005), loss='mean_squared_error')  # 损失函数用均方误差
